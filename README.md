...
## 기능 목록
- [X] 상품은 이름과 가격, 이미지를 가진다.
- [X] 상품 이미지는 파일이 아니라 URL을 직접 입력한다.
- [X] 상품을 조회/추가/수정/삭제할 수 있는 기능을 구현한다.
  - [X] 데이터는 컬렉션 프레임워크에 저장한다.
- [X] 상품을 추가하거나 수정하는 경우, 클라이언트로부터 잘못된 값이 전달되었는지 확인한다.
  - [X] 상품 이름은 공백을 포함하여 최대 15자까지 입력할 수 있다.
  - [X] 상품 이름에는 ( ), [ ], +, -, &, /, _ 외의 특수문자는 사용 불가능하다.
  - [X] 상품 이름에는 비속어를 포함할 수 없다. 
- [X] 메모리에 저장하고 있던 모든 코드를 제거하고 H2 데이터베이스를 사용하도록 변경한다.
- [X] 사용하는 테이블은 애플리케이션이 실행될 때 구축되어야 한다.
