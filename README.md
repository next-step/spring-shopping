...
### 4단계
- 주문과 관련된 도메인을 정의한다. 
  - [x] 주문 정보는 주문번호, 주문 아이템들, 총 결제금액으로 이루어져있다. 
  - [x] 주문 아이템 정보는 이름, 가격, 이미지, 수량으로 이루어져있다. 
- [x] 장바구니에 담긴 아이템 전체 주문할 수 있다
`POST /api/order`
- [x]  특정 주문의 상세 정보를 확인
...
- 주문 시점의 실시간 환율 저장 
  ...
  - [x] 실시간 환율  API :  https://currencylayer.com/ 
- 주문 관련 페이지에 적용 환율, 변환 금액 반영
  - [x] 주문 관련 페이지에 총 주문 금액과 함께 적용 환율, 변환된 총 주문 금액을 함께 노출